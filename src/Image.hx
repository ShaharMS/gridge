...
abstract Image(UInt8Array) {
	...
	**/
	public inline function new(width:Int, height:Int, ?color:Color = 0x00000000) {
		...
		var i = 4;
		while (i < this.length) {
			this[i] = color >> 24 & 0xFF;
			this[i + 1] = color >> 16 & 0xFF;
			this[i + 2] = color >> 8 & 0xFF;
			this[i + 3] = color & 0xFF;
			i += 4;
		}
	}
	...
	**/
	public inline function getFloatingPixel(x:Float, y:Float):Color {
		...
		if (!hasPixel(Math.ceil(x), Math.ceil(y)))
			throw #if vision new OutOfBounds(cast this, {x: x, y: y}) #else '($x, $y) is out of bounds' #end;
		#end
	...
	**/
	public inline function fillCircle(X:Int, Y:Int, r:Int, color:Color) {
        ...
        inline function min(values:Array<Int>) {
        ...
        inline function max(values:Array<Int>) {
		...
		for (y in 0...values.length) {
			...
			var min = min(values[y]);
			var max = max(values[y]);
	...
	public inline function resize(newWidth:Int, newHeight:Int, ?algorithm:ImageResizeAlgorithm):Image {
		...
		switch algorithm {
			case BilinearInterpolation:
				#if vision 
                this = cast vision.algorithms.BilinearInterpolation.interpolate(cast this, newWidth, newHeight);
                #else
                final resized = new Image(newWidth, newHeight);
		        final xMulitiplier = width / newWidth;
		        final yMulitiplier = height / newHeight;
		        resized.forEachPixel((x, y, c) -> {
		        	final color = getFloatingPixel(x * xMulitiplier, y * yMulitiplier);
			...
			case BicubicInterpolation:
				throw #if vision new Unimplemented("Bicubic Interpolation") #else "Bicubic Interpolation is not yet implemented." #end;
			case NearestNeighbor:
				{
					...
					forEachPixel((x, y, color) -> {
						var color = image.getPixel(Math.floor(x * xMulitiplier), Math.floor(y * yMulitiplier));
...
private class PixelIterator {
	...
	public inline function next():#if vision Pixel #else {x:Int, y:Int, color:Int} #end {
		final x = i % img.width;
		final y = Math.floor(i / img.width);
		var pixel:#if vision Pixel #else {x:Int, y:Int, color:Int} #end = {x: x, y: y, color: img.getPixel(x, y)};
		i += 4;
		return pixel;
	}
...
#if vision 
typedef IntPoint2D =  vision.ds.IntPoint2D;
#else
@:structInit
class IntPoint2D {
    public var x:Int;
...
typedef Color = #if vision vision.ds.Color #else Int #end;
typedef Rectangle = #if vision vision.ds.Rectangle #else {x:Int, y:Int, width:Int, height:Int} #end;
typedef Pixel = #if vision vision.ds.Pixel #else {x:Int, y:Int, color:Int} #end;
