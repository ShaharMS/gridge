...
class ImageTools {
	...
	**/
	public static function loadFromFile(?image:Image, path:String, onComplete:Image->Void) {
		...
		#if vision_quiet
		#else
		throw #if vision new Unimplemented(path.split(".").pop().toUpperCase() + "Decoding") #else path.split(".").pop().toUpperCase() + "Decoding is not yet implemented" #end;
		#end
		#else
	...
	**/
	...
	#else
	public static function grayscalePixel(pixel:Int):Int {
		final red = pixel >> 16 & 0xFF, green = pixel >> 8 & 0xFF, blue = pixel & 0xFF;
		final gray = Std.int((red + green + blue) / 3);
		return (pixel << 24 & 0xFF) | (gray << 16 & 0xFF) | (gray << 8 & 0xFF) | (gray & 0xFF);
	}
	#end
